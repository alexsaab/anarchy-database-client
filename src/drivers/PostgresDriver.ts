...
export class PostgresDriver extends BaseDriver {
  ...
  async getTableData(tableName: string, params: PageParams, schemaName: string = 'public'): Promise<QueryResult> {
    ...
    const query = buildPagedQuery({ dbType: 'PostgreSQL', tableRef, params, columns });

    const countSql = renumber(query.countSql, (i) => `$${i}`);
    ...
    const rowsSql = renumber(query.rowsSql, (i) => `$${i}`);
